...
public class CategoryController {
    ...
    @RequestMapping(value = {"/list-blog/{id}"}, method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<Blog>> bloglist(@PathVariable("id") Long id){
        Category category1 = categoryService.findById(id).orElse(null);
        if (category1 == null){
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else  {
            List<Blog> blogList = category1.getBlogList();
    ...
    public ResponseEntity<Category> edit(@PathVariable("id") Long id, @RequestBody Category category){
        Category currentCategory = categoryService.findById(id).orElse(null);
        if (category==null){
            return new ResponseEntity<Category>(HttpStatus.NOT_FOUND);
        }
        category.setId(currentCategory.getId());
        category.setNameCategory(currentCategory.getNameCategory());
